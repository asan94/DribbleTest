...
class BaseViewController: UIViewController {
    ...
    func showWithError(error: NSError) {
        ...
        showWhithTitleMessageCancelButtonActions(title: "Ошибка", message: error.localizedDescription as NSString, cancelButtonTitle: "OK")
    }
    
    func showWhithTitleMessageCancelButtonActions(title: NSString, message: NSString, cancelButtonTitle: NSString) {
        DispatchQueue.main.async {
            
            let alertViewController = UIAlertController(title:title as String, message: message as String, preferredStyle: .alert)
            let cancel = UIAlertAction(title: cancelButtonTitle as String, style: .cancel) { (action) -> Void in
